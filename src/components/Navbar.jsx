import { useState } from "react";
import LogoSi from "../assets/logo.png";

const Navbar = () => {
    const [dropdownVisible, setdropdownVisible] = useState (false);
    const [sidebardropdownVisible, setSidebardropdownVisible] = useState (false);
    const [sidebarVisible, setSidebarVisible] = useState (false);

    const handleDropdown = () => {
        setdropdownVisible(!dropdownVisible);
    };

    const handleSidebarDropdown = () => {
        setSidebardropdownVisible(!sidebardropdownVisible);
    };

    const handleSidebar = () => {
        setSidebarVisible(!sidebarVisible);
    };

    return(
        <div>
            <nav className="py-2 px-4 flex items-center justify-between">   
                <div className="flex flex-shrink-0">
                    <img className="w-[60px]" src={LogoSi} alt="Logo SI"></img>
                </div>
                <div className="hidden md:flex m-8 items-center gap-4 justify-center text-1xl">
                    <a href="">Home</a>
                    <a href="">Kabinet</a>
                    <a href="">Tentang</a>
                    <div>
                        <button className="flex " onClick={handleDropdown}>
                            Departemen
                            <svg
                                className="w-5 h-5"
                                width="20"
                                height="20"
                                viewBox="0 0 20 20"
                                fill="currentColor"
                                xmlns="http://www.w3.org/2000/svg"
                            >
                                <svg id="icons/regular/chevron-up-s">
                                    <path
                                        id="Icon"
                                        fill-rule="evenodd"
                                        clip-rule="evenodd"
                                        d="M14.4114 7.74408C14.7368 7.41864 15.2645 7.41864 15.5899 7.74408C15.9153 8.06951 15.9153 8.59715 15.5899 8.92259L10.5899 13.9226C10.2645 14.248 9.73683 14.248 9.4114 13.9226L4.4114 8.92259C4.08596 8.59715 4.08596 8.06951 4.4114 7.74408C4.73683 7.41864 5.26447 7.41864 5.58991 7.74408L10.0007 12.1548L14.4114 7.74408Z"
                                        fill="currentColor"
                                    />
                                </svg>
                            </svg>
                        </button>
                        {/* List Departemen */}
                        {dropdownVisible && (
                            <div className="absolute rounded flex flex-col mt-2">
                                <a href="">BPH</a>
                                <a href="">PSDM</a>
                                <a href="">PENRISTEK</a>
                                <a href="">MNB</a>
                                <a href="">KWU</a>
                                <a href="">DEPLU</a>
                                <a href="">DAGRI</a>
                                <a href="">DEPAG</a>
                                <a href="">KOMINFO</a>
                                <a href="">PE</a>
                            </div>
                        )}
                        {/* End List Departemen */}
                    </div>
                </div>

                <button className="block sm:hidden pr-2" onClick={handleSidebar}>
                    <svg
                    className="w-10 h-7 "
                    width="24"
                    height="24"
                    fill="none"
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                    >
                        <g id="bar-chart-2">
                            <path
                            id="Vector"
                            d="M20 6L10 6"
                            stroke="#F8F9FA"
                            stroke-width="2"
                            stroke-linecap="round"
                            stroke-linejoin="round"
                            />
                            <path
                            id="Vector_2"
                            d="M20 12L4 12"
                            stroke="#F8F9FA"
                            stroke-width="2"
                            stroke-linecap="round"
                            stroke-linejoin="round"
                            />
                            <path
                            id="Vector_3"
                            d="M20 18H14"
                            stroke="#F8F9FA"
                            stroke-width="2"
                            stroke-linecap="round"
                            stroke-linejoin="round"
                            />
                        </g>
                    </svg>
      </button>

                {/* side bar untuk tampilan mobile */}
                {sidebarVisible && (
                    <aside className="h-screen absolute top-0 right-0 bottom-0">
                    <div className="h-full border-r shadow-lg bg-white flex flex-col">
                        <button className="flex justify-end mr-2" onClick={handleSidebar}>
                            <img className="w-[30px] h-[30px]" 
                                src="data:image/svg+xml;base64," alt=""
                            />
                        </button>
                        <ul className="text-black mt-10 flex flex-col gap-1 pl-3 pr-10 items-start">
                            <li>
                                <a>Home</a>
                            </li>
                            <li>
                                <a>Kabinet</a>
                            </li>
                            <li>
                                <a>Tentang</a>
                            </li>
                            <li>
                                <button className="flex flex-row" onClick={handleSidebarDropdown}>
                                    Departemen
                                    <svg
                                        className="w-5 h-5"
                                        width="20"
                                        height="20"
                                        viewBox="0 0 20 20"
                                        fill="currentColor"
                                        xmlns="http://www.w3.org/2000/svg"
                                    >
                                        <svg id="icons/regular/chevron-up-s">
                                            <path
                                                id="Icon"
                                                fill-rule="evenodd"
                                                clip-rule="evenodd"
                                                d="M14.4114 7.74408C14.7368 7.41864 15.2645 7.41864 15.5899 7.74408C15.9153 8.06951 15.9153 8.59715 15.5899 8.92259L10.5899 13.9226C10.2645 14.248 9.73683 14.248 9.4114 13.9226L4.4114 8.92259C4.08596 8.59715 4.08596 8.06951 4.4114 7.74408C4.73683 7.41864 5.26447 7.41864 5.58991 7.74408L10.0007 12.1548L14.4114 7.74408Z"
                                                fill="currentColor"
                                                />
                                        </svg>
                                    </svg>
                                </button>
                                {/* list departemen mobile */}
                                {sidebardropdownVisible && (
                                    <ul className="ml-2 mt-1 text-sm">
                                        <li><a href="">BPH</a></li>
                                        <li><a href="">PSDM</a></li>
                                        <li><a href="">PENRISTEK</a></li>
                                        <li><a href="">MNB</a></li>
                                        <li><a href="">KWU</a></li>
                                        <li><a href="">DEPLU</a></li>
                                        <li><a href="">DAGRI</a></li>
                                        <li><a href="">DEPAG</a></li>
                                        <li><a href="">KOMINFO</a></li>
                                    </ul>
                                )}
                                {/* end list departemen mobile */}
                            </li>
                        </ul>
                    </div>
                    </aside>  
                )}
                {/* end side bar untuk tampilan mobile */}
            </nav>
        </div>
    );
};

export default Navbar;